package com.mytest.utils.security.des3;
...
import com.mytest.utils.security.MessageAuthenticationCode;
...
public class Des3Encryption {
	...
	public static String decryptFromHexEcb(String key, String value){
		try {
			byte[] keyByte = key.getBytes(CHAR_ENCODING);
			byte[] valueByte = ConvertUtils.fromHex(value);
			byte[] dataByte = MessageAuthenticationCode.des3DecryptionEcb(keyByte, valueByte);
			String data = new String(dataByte, CHAR_ENCODING);
			return data;
		} catch (Exception e) {
			return null;
